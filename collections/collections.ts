...
module collections {
    ...
    */
    ...
    /**
    * Joins all the properties of the object using the provided join string
    */
    ...
    export function makeJson<T>(item: T, join: string = ","): string {
        if (item === null) {
            return 'COLLECTION_NULL';
        } else if (collections.isUndefined(item)) {
            return 'COLLECTION_UNDEFINED';
            ...
            for (var prop in item) {
                if (has(item, prop)) {
                    if (first)
                        ...
                        toret = toret + join;
                    //item[prop].forEach(function(e){console.log(e)} )
